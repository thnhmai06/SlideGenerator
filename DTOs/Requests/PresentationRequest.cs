...
public record ScanShapesRequest(string FilePath)
    : PresentationRequest(PresentationRequestType.ScanShapes), IFilePathBased;
...
public record GenerateSlideGroupCreateRequest(
    ...
    string[]? CustomSheet) : PresentationRequest(PresentationRequestType.GroupCreate), IFilePathBased;

public record GenerateSlideGroupStatusRequest(string FilePath)
    : PresentationRequest(PresentationRequestType.GroupStatus), IFilePathBased;

public record GenerateSlideGroupControlRequest(string FilePath, ControlState? State = null)
    : PresentationRequest(PresentationRequestType.GroupControl), IFilePathBased;

#endregion

#region Job

public record GenerateSlideJobControlRequest(string JobId, ControlState? State = null)
    : PresentationRequest(PresentationRequestType.JobControl), IJobBased;
