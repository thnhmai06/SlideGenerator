...
/**
 * Parse solution projects and project references from the backend solution.
...
function parseSolution(backendDir) {
    const solution = fs.readFileSync(path.join(backendDir, 'SlideGenerator.slnx'), 'utf8');
    const projPaths = [...solution.matchAll(/<Project\s+Path="([^"]+)"/g)].map(m => m[1]);
...
/**
 * Resolve changed the file path to owning project by directory containment.
 * @param {string} ws GitHub workspace path.
 * @param {Map<string, string>} nameToDir Project name to absolute project directory.
 * @param {string} file Workspace-relative changed file path.
 * @returns {string|null} Owning project name, or null when the file is outside projects.
 */
