...
function getCaseInsensitive<T extends Record<string, unknown>, TValue = unknown>(
...
export async function loadFile(filePath: string): Promise<LoadFileResponse> {
  ...
  const sheets = (getCaseInsensitive<Record<string, number>>(tables, 'Sheets') ?? {}) as Record<
    string,
    number
  >
...
export async function getSheets(filePath: string): Promise<SheetListResponse> {
  ...
  const tables = (getCaseInsensitive<Record<string, number>>(data, 'Sheets') ?? {}) as Record<
    string,
    number
  >
...
export async function getSheetData(
  ...
  for (let rowIndex = startRow; rowIndex <= endRow; rowIndex += 1) {
    ...
    const rowData = (getCaseInsensitive<Record<string, string | null>>(row, 'Row') ??
  ...
  for (const filePath of filePaths) {
    try {
      ...
      sheets.forEach((sheet) => {
        const headers = (getCaseInsensitive<Array<string | null>>(sheet, 'Headers') ??
          []) as Array<string | null>
        headers.filter((header): header is string => Boolean(header)).forEach(
          (header) => allColumns.add(header)
        )
      })
    } catch (error) {
...
export async function scanShapes(filePath: string): Promise<unknown> {
  ...
  return {
    ...
    Shapes: ((getCaseInsensitive<Array<Record<string, unknown>>>(data, 'Shapes') ?? []) as Array<
...
export async function createGroup(request: Record<string, unknown>): Promise<unknown> {
  const response = await slideHub.sendRequest<ResponseBase>({
    type: 'groupcreate',
    ...request,
  })
  ...
  return {
    ...
    JobIds:
      (getCaseInsensitive<Record<string, string>>(data, 'JobIds') ?? {}) as Record<
        string,
        string
      >,
...
export async function groupStatus(request: Record<string, unknown>): Promise<unknown> {
  const response = await slideHub.sendRequest<ResponseBase>({
    type: 'groupstatus',
    ...request,
  })
  ...
  const rawJobs =
    (getCaseInsensitive<Record<string, Record<string, unknown>>>(data, 'Jobs') ??
...
export async function groupControl(request: Record<string, unknown>): Promise<unknown> {
  const response = await slideHub.sendRequest<ResponseBase>({
    type: 'groupcontrol',
    ...request,
  })
...
export async function jobControl(request: Record<string, unknown>): Promise<unknown> {
  const response = await slideHub.sendRequest<ResponseBase>({
    type: 'jobcontrol',
    ...request,
  })
...
export async function getAllGroups(): Promise<unknown> {
  ...
  const rawGroups =
    (getCaseInsensitive<Array<Record<string, unknown>>>(data, 'Groups') ?? []) as Array<
      Record<string, unknown>
    >
  return {
    Type: getResponseType(data),
