...
type ImageReplacementPanelProps = {
  ...
  getOptionDescription: (options: { value: string; description: string }[], value: string) => string
  columns: string[]
...
const CreateTaskMenu: React.FC<CreateTaskMenuProps> = ({ onStart }) => {
  ...
  const resolveHydrationAssets = async (
    ...
    const dataAssets =
      !cached.dataLoaded && nextDataPath
        ? await loadDataAssets(nextDataPath)
        : { columns: cached.columns, sheetCount: cached.sheetCount, totalRows: cached.totalRows }
  ...
  return (
    <div className="input-menu">
      ...
      <StartButtonSection isStarting={isStarting} canStart={canStart} onStart={handleStart} t={t} />
