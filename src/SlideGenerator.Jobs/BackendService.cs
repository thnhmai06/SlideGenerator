...
using SlideGenerator.Scanning.Services;
...
public sealed class BackendService : IAsyncDisposable
{
    ...
    /// </summary>
    public async Task<Guid> CreateJobAsync(GenerateSlidesRequest request, CancellationToken cancellationToken)
    {
...
ORDER BY sheet_name;";
    ...
    private async Task RunBookJobAsync(Guid jobId, JobControlState state, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            using var workbook = WorkbookService.OpenWorkbook(request.Sheet.FilePath);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var template = request.TemplateMap.TryGetValue(sheetName, out var sheetTemplate)
            ? sheetTemplate
        ...
        using var document = XmlPresentationService.OpenOrCreatePresentation(outputPath, true);
    ...
    private async Task<GenerateSlidesRequest?> LoadRequestAsync(Guid jobId, CancellationToken cancellationToken)
    {
        await using var connection = OpenConnection();
        await connection.OpenAsync(cancellationToken);
        ...
        return string.IsNullOrWhiteSpace(json)
            ? null
            : JsonSerializer.Deserialize<GenerateSlidesRequest>(json);
    }
