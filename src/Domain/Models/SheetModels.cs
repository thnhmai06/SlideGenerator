    ...
    IDisposable
{
    ...
    public Workbook(string filePath)
    {
        ...
        if (!BackendConfig.SpreadsheetExtensions.Contains(ext))
...
public class Worksheet : Model
{
    ...
    public Dictionary<string, object?> GetRow(int index)
    {
        if (index < 1 || index > RowCount)
            throw new IndexOutOfRangeException($"Index {index} is out of range [1, {RowCount}]");
