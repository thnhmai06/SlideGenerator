    ...
    IFileSystem fileSystem) : Service(logger), IJobExecutor
{
    ...
    private bool TryGetSheetAndGroup(string sheetId, out JobSheet? sheet, out JobGroup? group)
    {
        ...
        if (group == null)
        {
            Logger.LogWarning("Group {GroupId} not found for job {JobId}{HangfireSuffix}", sheet.GroupId,
                sheetId, FormatHangfireSuffix(sheet.HangfireJobId));
            return false;
        }
    ...
    private async Task CompleteJobAsync(JobSheet sheet, JobGroup group, string sheetId)
    {
        ...
        Logger.LogInformation("Job {JobId}{HangfireSuffix} completed successfully", sheetId,
            FormatHangfireSuffix(sheet.HangfireJobId));
        ...
        List<JobLogEntry>? bufferedLogs)
    {
        ...
        Logger.LogInformation("Job {JobId}{HangfireSuffix} was paused/cancelled", sheetId,
            FormatHangfireSuffix(sheet.HangfireJobId));
        ...
        List<JobLogEntry>? bufferedLogs)
    {
        ...
        Logger.LogError(ex, "Job {JobId}{HangfireSuffix} failed", sheetId,
            FormatHangfireSuffix(sheet.HangfireJobId));
