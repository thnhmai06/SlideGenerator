...
function normalizeJobStatusInfo(input: Record<string, unknown>): JobStatusInfo {
  return {
    JobId: (() => {
      const val = getCaseInsensitive(input, 'JobId')
    ...
    Status: (() => {
      const val = getCaseInsensitive(input, 'Status')
      return typeof val === 'string' ? val : ''
    })(),
...
function normalizeGroupSummary(input: Record<string, unknown>): GroupSummary {
  return {
    ...
    WorkbookPath: (() => {
      const val = getCaseInsensitive(input, 'WorkbookPath')
      return typeof val === 'string' ? val : ''
    })(),
    OutputFolder: (() => {
...
function normalizeJobLogEntry(input: Record<string, unknown>): JobLogEntry {
  return {
    Level: (() => {
      const val = getCaseInsensitive(input, 'Level')
      return typeof val === 'string' ? val : ''
    })(),
    Message: (() => {
      const val = getCaseInsensitive(input, 'Message')
      return typeof val === 'string' ? val : ''
    })(),
    Timestamp: (() => {
      const val = getCaseInsensitive(input, 'Timestamp')
      return typeof val === 'string' ? val : ''
    })(),
...
export async function createGroup(
  ...
  return {
    ...
    JobIds: (getCaseInsensitive<Record<string, string>>(data, 'JobIds') ?? {}) as Record<
      string,
      string
    >,
...
export async function groupStatus(
  ...
  const rawJobs = (getCaseInsensitive<Record<string, Record<string, unknown>>>(data, 'Jobs') ??
...
export async function jobStatus(request: Record<string, unknown>): Promise<SlideJobStatusSuccess> {
  ...
  return {
    ...
    HangfireJobId: getCaseInsensitive<string>(data, 'HangfireJobId') ?? undefined,
...
export async function getJobLogs(request: Record<string, unknown>): Promise<SlideJobLogsSuccess> {
  ...
  const logs = (getCaseInsensitive<Array<Record<string, unknown>>>(data, 'Logs') ?? []) as Array<
    Record<string, unknown>
  >
  return {
    Type: 'joblogs',
...
export async function getAllGroups(): Promise<SlideGlobalGetGroupsSuccess> {
  ...
  const rawGroups = (getCaseInsensitive<Array<Record<string, unknown>>>(data, 'Groups') ??
    []) as Array<Record<string, unknown>>
  return {
    Type: 'getallgroups',
