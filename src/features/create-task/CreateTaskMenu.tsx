...
const buildSheetInfo = (sheetsInfo: Array<{ Name?: string | null; RowCount?: number | null }>) => {
...
const importConfigFromFile = async (args: {
	...
	setLastLoadedTemplatePath: (path: string) => void;
	setLastLoadedDataPath: (path: string) => void;
	showNotification: (type: 'success' | 'error', text: string) => void;
	t: (key: string) => string;
}) => {
	...
	try {
		...
		if (nextDataPath) {
			...
			args.setLastLoadedDataPath(nextDataPath);
		}
			...
			<div className="panel-header">
				...
				<button
					...
					disabled={
						...
						textReplacements.length >= maxTextConfigs
					}
					title={
						isAtLimit
							? `${t('replacement.limitReached')}: ${maxTextConfigs}`
							: undefined
					}
				>
					+ {t('replacement.add')}
			...
			<div className="panel-header">
				...
				<button
					...
					disabled={
						!canConfigure ||
						shapes.length === 0 ||
						imageReplacements.length >= maxImageConfigs
					}
					title={
						isAtLimit
							? `${t('replacement.limitReached')}: ${maxImageConfigs}`
							: undefined
					}
				>
					+ {t('replacement.add')}
				...
				<div className="replacement-table replacement-table-image">
					<div className="shape-gallery">
						...
						<div className="shape-gallery-list">
							{shapes.length === 0 ? (
								<div className="shape-gallery-empty">
	...
	const importConfig = useCallback(
		() =>
			importConfigFromFile({
				...
				setIsLoadingColumns,
				setLastLoadedTemplatePath: (path) => {
					lastLoadedTemplatePathRef.current = path;
				},
				setLastLoadedDataPath: (path) => {
					lastLoadedDataPathRef.current = path;
				},
				showNotification,
				t,
			}),
		[
			...
			setIsLoadingPlaceholders,
			setIsLoadingColumns,
			lastLoadedTemplatePathRef,
			lastLoadedDataPathRef,
			showNotification,
			t,
		],
	);
				...
				<div className="sheet-selector">
					<div className="sheet-selector-header">
						...
						<span className="sheet-selector-count">
							{t('createTask.sheetSelected')}: {selectedSheets.length}/
							{sheetNames.length}
						</span>
					</div>
					<div
