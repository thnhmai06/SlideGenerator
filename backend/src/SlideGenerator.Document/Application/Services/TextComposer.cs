...
///     <list type="number">
///         <item>
///             Flatten all TextParts across all paragraphs into a single logical string,
///             tracking each TextPart's character range (PartSpan).
///         </item>
///         <item>
///             Use a two-pointer scan to locate Mustache tag boundaries and classify tags
///             as Renderable (variables) or Structural (sections/loops).
///         </item>
///         <item>
///             Build a marked template: wrap Renderable and static-text segments with unique
///             XML-like markers; inject paragraph-break sentinels; pass Structural tags through.
///         </item>
...
///         <item>
///             Reconstruct the shape: for each chunk, create a new paragraph and distribute
///             the rendered text across TextParts proportionally (Coverage Ratio).
///         </item>
///     </list>
/// </remarks>
public sealed partial class TextComposer(ITemplateEngine templateEngine) : ITextComposer
{
    ...
    /// <param name="partSpans">
    ///     A list of part spans indicating the mapping of text parts to their positions in the flattened
    ///     string.
    /// </param>
    ...
    /// <returns>
    ///     A list of segments, where each segment contains metadata
    ...
    /// <returns>
    ///     A <see cref="SegmentKind" /> enum value indicating the type of the tag: either
    ///     <see cref="SegmentKind.StructuralTag" /> or <see cref="SegmentKind.RenderableTag" />.
    /// </returns>
    ...
    /// <returns>
    ///     A list of tuples where each tuple contains a text part of the source and the corresponding segment of the text
    ///     content.
    /// </returns>
    ...
    ///     coverage entries. Each coverage entry maps original text parts to their fractional ownership in the marker
    ///     segments.
    /// </param>
        ...
        IReadOnlyDictionary<string, List<CoverageEntry>> markerMap)
    {
        ...
        while (i < chunk.Length)
        {
            ...
            if (!IsHex32(chunk, i + 2) || chunk[i + 34] != '>')
            {
                i++;
                continue;
            }
