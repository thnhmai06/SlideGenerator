...
public interface IGeneratingService
{
    ...
    /// <summary>
    ///     Returns <see langword="true" /> if a workflow instance using the given recipe is currently active
    ///     (running or paused), <see langword="false" /> otherwise.
    /// </summary>
    Task<bool> IsRecipeInUseAsync(int recipeId, CancellationToken ct = default);
}