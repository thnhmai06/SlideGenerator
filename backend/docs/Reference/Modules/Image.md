...
- **Performance**: MagickImage is converted to an OpenCV `Mat` internally for detection, then the resulting coordinates
  are used to crop the original MagickImage.

## MagickImage Factory

Provides a unified way to open images from `byte[]` or file paths, ensuring consistent decoding settings across the
application.
