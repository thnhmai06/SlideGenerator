...
The **SlideGenerator.Cloud** module merges multi-cloud URI resolution with HTTP-based asset acquisition. It replaces the
previous separate `Resolver` and `Download` modules.
...
OneDrive, SharePoint, and other providers from older drafts are **not yet implemented** in the current code — only
Google Drive is wired.
...
The `CollectImage` step in the Generator uses `ICloudClient` to fetch each image into the workflow's temp folder, gated
by `GateType.DownloadImage`.
