...
/// <summary>
///     A strongly typed identifier for a workflow-scoped variable handle.
///     Instances carry no data — they serve only as typed keys into the
///     <see cref="IExecutionContext" /> scope dictionary.
///     Because <see cref="Handle{T}" /> is stateless, its instances are safe to declare
...
public sealed record Handle<T>(string Name);